...
async function get_latest_proposal() {
    ...
    for (let i = 0; i < forum_urls.length; i++) {
        ...
        latest_prop = latest_prop.concat(latest_prop,parse_response(await response.json(), forum_urls[i]["forum_name"], forum_urls[i]["proposal_link"]))
    }
    return latest_prop
}
...
function parse_response(response, dao_name, proposal_link) {
    ...
    for (let i = 0; i < response["topic_list"]["topics"].length; i++) {
        if (response["topic_list"]["topics"][i]["title"].search("proposal") !== -1 || response["topic_list"]["topics"][i]["title"].search("Proposal") !== -1 || response["topic_list"]["topics"][i]["title"].search("PROPOSAL") !== -1) {
            proposals.push({
                "dao": dao_name,
